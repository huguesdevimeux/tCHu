...
public final class Game {
    ...
    public static void play(Map<PlayerId, Player> players, Map<PlayerId, String> playerNames, SortedBag<Ticket> tickets, Random rng) {
        ...
        players.forEach((playerId, player) -> player.receiveInfo(new Info(firstPlayer.name()).willPlayFirst()));
        ...
        players.forEach(
                (playerId, player) ->
                        player.receiveInfo(new Info(playerId.name()).keptTickets(tickets.size() - player.chooseInitialTickets().size())));
    }
}
