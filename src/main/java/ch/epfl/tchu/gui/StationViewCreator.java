    ...
    public static void createStationsView(Pane gameMapPane, ListView<Ticket> ListView) {
        ...
        ListView.setOnMouseClicked(
                e -> {
                    ...
                    // we create a new list that only will contain at most 2 circles, that will be
                    // the from
                    ...
                    // {Allemagne (6),
                    // France (10), ...}" and in that case, only the station COIRE will be
                    // highlighted.
                    switch (newList.size()) {
                        case 1:
                        ...
                        case 2:
                            ...
                            // Lugano) will have
                            // Lugano as the from station as it comes first alphabetically, but it
                            // is wrong.
                            fromStation =
                                    newList.get(0)
                                                    .getId()
                                                    .equals(
                                                            ChMap.normalizedStations()
                                                                    .get(
                                                                            ChMap.stations()
                                                                                    .indexOf(from)))
                                            ? newList.get(0)
                            ...
                            // we set default colors for the from and to stations: blue-ish = from,
                            // red-ish = to.
                            fromStation.setStyle(SWISS_FROM_STATION_STYLE);
                            toStation.setStyle(SWISS_TO_STATION_STYLE);
                            // each circle representing the from and two stations will have their
                            // radius increased
        ...
        // On mouse exited, the style is reset to white and the radius is reset to 6, the initial
        // radius.
