...
final class MapViewCreator {
   ...
   * @return the node that contains all elements in the mapView - an instance of Pane in this case.
   */
    ...
    for (Route route : ChMap.routes()) {
      ...
      mainRouteGroup
          ...
          .addAll(
              STYLE_CLASS_ROUTE,
              route.level().name(),
              route.color() == null ? STYLE_CLASS_COLOR_NEUTRAL : route.color().name());
      ...
      mainRouteGroup.setOnMouseClicked(
          event -> {
            List<SortedBag<Card>> possibleClaimCards = obsGameState.possibleClaimCards(route).get();
