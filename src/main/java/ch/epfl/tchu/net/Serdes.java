    ...
    public static Serde<PublicGameState> publicGameStateSerde = Serde.of(
            i ->
                    String.join(COLON_SEPARATOR,
                            ...
                            i.lastPlayer() == null ? stringSerde.serialize("") : playerIdSerde.serialize(i.lastPlayer()))

            , s -> {
                String[] elements = s.split(Pattern.quote(COLON_SEPARATOR), -1);
                PublicPlayerState firstPlayerState = publicPlayerStateSerde.deserialize(elements[3] + "0");
                PublicPlayerState secondPlayerState = publicPlayerStateSerde.deserialize(elements[4] + "0");
                PlayerId lastPlayer = elements[5].isEmpty() ? null : playerIdSerde.deserialize(elements[5]);
                Map<PlayerId, PublicPlayerState> map = Map.of(PLAYER_1, firstPlayerState, PLAYER_2, secondPlayerState);
                return new PublicGameState(intSerde.deserialize(elements[0]),
