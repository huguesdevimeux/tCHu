...
public final class RemotePlayerProxy implements Player {
            ...
            this.writer =
                    new BufferedWriter(
                            new OutputStreamWriter(
                                    socket.getOutputStream(), Constants.Network.ENCODING));
            this.reader =
                    new BufferedReader(
                            new InputStreamReader(socket.getInputStream(), Constants.Network.ENCODING));
        } catch (IOException e) {
    ...
    private void sendInNetwork(MessageId messageId, List<String> serializedArgs) {
        try {
            this.writer.write(Objects.requireNonNull(messageId).name() + Constants.Network.SEPARATOR_COMPONENT_MESSAGE);
            if (serializedArgs.size() > 0) {
                this.writer.write(
                        String.join(Constants.Network.SEPARATOR_COMPONENT_MESSAGE, Objects.requireNonNull(serializedArgs)));
            }
            this.writer.write(Constants.Network.CHAR_END_MESSAGE);
            this.writer.flush();
        } catch (IOException e) {
            throw new UncheckedIOException(e);
        }
